...
class PDFDocument:
    ...
    PAGE_SIZES_MM = {
        'A0': (841.0,1189.0),'A1':(594.0,841.0),'A2':(420.0,594.0),'A3':(297.0,420.0),'A4':(210.0,297.0),'A5':(148.0,210.0),
        'B0':(1000.0,1414.0),'B1':(707.0,1000.0),'B2':(500.0,707.0),'B3':(353.0,500.0),'B4':(250.0,353.0),'B5':(176.0,250.0),
        'Letter':(215.9,279.4),'Legal':(215.9,355.6),
    }

    def page_size_label(self,page_index):
        page=self.get_page(int(page_index))
        if page is None: return ''
        w=float(page.rect.width)*25.4/72.0; h=float(page.rect.height)*25.4/72.0
        s,l=min(w,h),max(w,h); best=None; err=float('inf')
        for name,dims in self.PAGE_SIZES_MM.items():
            e=max(abs(s-min(dims)),abs(l-max(dims)))
            if e<err: best,err=name,e
        return best if best and err<=4.0 else f'{round(w)}×{round(h)}'

    def _page_size_points(self,size_name,orientation,current_rect):
        if size_name not in self.PAGE_SIZES_MM: raise PDFSaveError('指定された用紙サイズに対応していません。')
        w,h=self.PAGE_SIZES_MM[size_name]
        landscape=(current_rect.width>current_rect.height) if orientation=='current' else orientation=='landscape'
        if landscape: w,h=max(w,h),min(w,h)
        else: w,h=min(w,h),max(w,h)
        return w*72/25.4,h*72/25.4

    def resize_page(self,page_index,size_name,orientation='current',scale_content=True):
        if self.doc is None: raise PDFSaveError('PDFが開かれていません。')
        page_index=int(page_index)
        if not 0<=page_index<len(self.doc): raise PDFSaveError('指定されたページが存在しません。')
        source=self.doc[page_index]; tw,th=self._page_size_points(str(size_name),str(orientation),source.rect)
        if not scale_content:
            source.set_mediabox(fitz.Rect(0,0,tw,th)); self.structure_dirty=True; return
        rebuilt=fitz.open()
        try:
            for i in range(len(self.doc)):
                if i!=page_index: rebuilt.insert_pdf(self.doc,from_page=i,to_page=i); continue
                target=rebuilt.new_page(width=tw,height=th)
                target.show_pdf_page(fitz.Rect(0,0,tw,th),self.doc,i,keep_proportion=True,overlay=True)
            old=self.doc; self.doc=rebuilt; rebuilt=None; old.close(); self.structure_dirty=True
        except Exception as error:
            raise PDFSaveError('ページサイズの変更に失敗しました。') from error
        finally:
    ...
    def add_checkmark(self, page_index, x, y, size=15.0, color="#dc0000", line_width=2.2):
        ...
        for px, py in display_points:
            point = self._display_point_to_pdf(
                page,
                px,
                py,
            )
        ...
        # Store the two arms as separate open ink strokes. This avoids
        # PDF viewers or printer drivers treating the three-point path as
        # a closed triangular polygon.
        strokes = [
            [
                points[0],
                points[1],
        ...
        rgb = self._parse_annotation_color(
            color,
            fallback=(0.86, 0.0, 0.0),
        )
    ...
    def add_freetext_comment(
        ...
        border_color="#dc0000",
        border_width=1.5,
        border_padding_x=2.0,
        border_padding_y=1.0,
    ):
        page = self.get_page(page_index)
        if page is None:
            return None
        ...
        size = max(float(font_size), 6.0)
        padding_x = max(
            float(border_padding_x),
            0.0,
        )
        padding_y = max(
            float(border_padding_y),
            0.0,
        )
        width = max(
            18.0,
        ...
        border_rgb = self._parse_annotation_color(
            border_color,
            fallback=(0.86, 0.0, 0.0),
        )
    ...
    def add_freehand(
        ...
        for raw in points:
            if not (
                ...
                continue
            display_x = float(x) + float(raw[0])
            display_y = float(y) + float(raw[1])
            point = self._display_point_to_pdf(
                page,
                display_x,
                display_y,
